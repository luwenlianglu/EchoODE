...
class ConvGRU_ODE(nn.Module):
    def __init__(self, input_dim, hidden_dim, time_dilation = 2, timesteps = 4, kernel_size=(3,3), batch_first=True,
        ...
        super(ConvGRU_ODE, self).__init__()
    ...
    def build_model(self):
        ...
        gen_ode_func = ODEFunc(input_dim=self.ode_dim,
                               latent_dim=self.ode_dim,  # channels after encoder, & latent dimension
    ...
    def forward(self, input_tensor, pred_dur = False, input_time_steps=torch.linspace(0, 1, 4), time_steps_to_pred=torch.linspace(0, 1, 4)):
        ...
        first_point_mu, first_point_std = self.encoder_z0(input_tensor, input_time_steps)
...
if __name__ == "__main__":
    ...
    convgruodenet = ConvGRU_ODE(input_dim=64, hidden_dim=64, kernel_size=(3,3), batch_first=True,
                 bias=True, return_all_layers=True)

    x = torch.randn((2, 4, 64, 14, 14))
    true_time_steps = torch.tensor([0,0.2,0.4,0.6])
    time_steps_pred = torch.tensor([0,0.2,0.4,0.6, 0.8,1])
    y = convgruodenet(x)
