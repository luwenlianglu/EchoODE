...
class Discriminator(nn.Module):
    ...
    def netD_adv_loss(self, real, fake, device):
        
        if self.seq:
            if self.is_extrap:
                real, fake = self.rearrange_seq(real, fake, only_fake=False)
            else:
                real, fake = self.rearrange_seq_interp(real, fake,device, only_fake=False)
        elif not self.seq:
            b, t, c, h, w = fake.size()
    ...
    def rearrange_seq(self, real, fake, only_fake=True):
        
        b, t, c, h, w = fake.size()
        fake_seqs = []
        for i in range(t):
            fake_seq = torch.cat([real[:, i:, ...], fake[:, :i+1, ...]], dim=1)
            fake_seqs += [fake_seq]
        ...
        real_seqs = []
        for i in range(t):
            real_seq = torch.cat([real[:, i:, ...], real[:, :i+1, ...]], dim=1)
            real_seqs += [real_seq]
        real_seqs = torch.cat(real_seqs, dim=0).view(b * t, -1, h, w)
        
        return real_seqs, fake_seqs

    def rearrange_seq_interp(self, real, fake,device, only_fake=True):
    
        b, t, c, h, w = fake.size()
        mask = torch.eye(t).float().to(device)
        fake_seqs = []
        for i in range(t):
            reshaped_mask = mask[i].view(1, -1, 1, 1, 1)
            fake_seq = (1 - reshaped_mask) * real + reshaped_mask * fake
            fake_seqs += [fake_seq]
        fake_seqs = torch.cat(fake_seqs, dim=0).view(b * t, -1, h, w)
        ...
        for i in range(t):
            reshaped_mask = mask[i].view(1, -1, 1, 1, 1)
            real_seq = (1 - reshaped_mask) * real + reshaped_mask * real
            real_seqs += [real_seq]
        real_seqs = torch.cat(real_seqs, dim=0).view(b * t, -1, h, w)
...
def create_netD(opt):
    ...
    # Model
    seq_len = opt.timesteps

    netD_img = Discriminator(in_ch=3, device=opt.device, seq=False, is_extrap=False).to(opt.device)
    netD_seq = Discriminator(in_ch=3 * (seq_len), device=opt.device, seq=True, is_extrap=False).to(opt.device)
